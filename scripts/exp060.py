...
@dataclass
class CFG:
    exp_name: str = "exp060_2_stackedUnet"
    mode = ["train", "test"]
    debug: bool = False
    ...
    train_fold: list[int] = field(default_factory=lambda: [1, 2, 3, 4])
    grad_accum: int = 1
    patience: int = 10
    is_second_stage: bool = True
    use_amp: bool = True
    loss: str = "BCESoftDiceLoss"
    # loss: str = "SoftBCE"
    """BCESoftDiceLoss, DiceLoss, BCEDiceLoss, BCEWithLogitsLoss, TverskyLoss, SoftBCE"""
    bce_weight: float = 1.0  # BCEWithLogitsLossのweight
    ...
    # swa_start: int = 10
    swa_epoch_range: tuple[int, int] = (12, 20)
    # swa_lr: float = 1e-7
    swa_lr: float = 1e-6
    swa_epoch_num: int = 10
    ...
    resume_weights: list[str | Path] = field(
        default_factory=lambda: [
            "output/exp060_2_stackedUnet/checkpoint_1.pth",
            # "output/exp060_stackedUnet/checkpoint_2.pth",
            # "output/exp060_stackedUnet/checkpoint_3.pth",
            # "output/exp060_stackedUnet/checkpoint_4.pth",
            # "output/exp060_stackedUnet/checkpoint_5.pth",
        ]
    )

    # --- Data Config
    # fragment_z: tuple[int, int] = (0, 32)
    # fragment_z: tuple[int, int] = (28, 38)
    # fragment_z: tuple[int, int] = (24, 40)
    # fragment_z: tuple[int, int] = (18, 37)
    # fragment_z: tuple[int, int] = (16, 34)
    fragment_z: tuple[int, int] = (25, 40)

    """should be 0 ~ 65, [z_min, z_max)"""
    fragment_depth: int = 10
    # crop_size: int = 384
    # crop_size: int = 256
    crop_size: int = 768
    """same as tile size."""
    ...
    train_compose = [
        ...
        A.Normalize(mean=[0] * int(fragment_z[1] - fragment_z[0]), std=[1] * int(fragment_z[1] - fragment_z[0])),
    ...
    soft_train_compose = [
        ...
        A.VerticalFlip(p=0.5),
        A.RandomRotate90(always_apply=True),
        A.ShiftScaleRotate(shift_limit=0.1, scale_limit=0.2, rotate_limit=360, p=0.5),
        A.OneOf(
            [
                A.GaussNoise(var_limit=50),
                A.GaussianBlur(),
...
def read_image_mask(cfg: CFG, fragment_id: int, fragment_z: tuple[int, int]) -> tuple[np.ndarray, np.ndarray]:
    ...
    logger.info(f"volume.min() = {volume.min()}, volume.max() = {volume.max()}")
    ...
    mask /= 255.0
    logger.info(f"mask.min() = {mask.min()}, mask.max() = {mask.max()}")
...
def get_train_valid_split(
    ...
    if cfg.debug:
        ...
        valid_masks = valid_masks[:100]
        valid_xyxys = valid_xyxys[:100]
...
def get_train_loader(cfg: CFG, train_images: list[np.ndarray], train_labels: list[np.ndarray]) -> PrefetchLoader:
    train_batch_size = cfg.batch_size
    valid_batch_size = cfg.valid_batch_size
    num_workers = cfg.num_workers
    fragment_depth = cfg.fragment_z[1] - cfg.fragment_z[0]
    use_amp = cfg.use_amp
...
class VCNet(nn.Module):
    ...
    def forward(self, volume: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        Args:
            volume: (batch_size, crop_depth, height, width),
...
def training_fn(cfg: CFG) -> None:
    ...
    swa_epoch_range = cfg.swa_epoch_range
    swa_lr = cfg.swa_lr
    swa_epoch_num = cfg.swa_epoch_num

    for fold in range(1, n_fold + 1):
        if fold not in train_folds:
            continue
        if cfg.full_train and fold > 1:
            break
        ...
        if resume_train:
            logger.info(f"resume from {resume_weights[fold - 1]}")
            net.load_state_dict(torch.load(resume_weights[fold - 1], map_location=device))

        if cfg.full_train:
            logger.info(f"Before: {len(train_images)}, {len(valid_images)}")
            train_images.extend(valid_images)
            train_labels.extend(valid_labels)
            logger.info(f"After: {len(train_images)}")
            train_loader = get_train_loader(cfg, train_images, train_labels)
        else:
        ...
        swa_scheduler = SWALR(optimizer, swa_lr=swa_lr, anneal_strategy="cos", anneal_epochs=swa_epoch_num)
        best_score = 0
        use_awp = False
        ...
        for epoch in range(epoch):
            if cfg.full_train:
                ...
                wandb.log(
                    {
                        f"full_train_avg_loss": train_avg_loss,
                    }
                )
            ...
            if schedule_per_epoch:
                if swa_epoch_range[0] <= epoch < swa_epoch_range[1]:
        ...
        train_loader.fp16 = False
        torch.optim.swa_utils.update_bn(train_loader, swa_model)
        early_stopping.save_checkpoint(val_loss=0, model=swa_model, prefix="last-swa-")
        if cfg.full_train:
            early_stopping.save_checkpoint(val_loss=0, model=net, prefix="full-")
        del net
        gc.collect()
        ...
        if not cfg.full_train:
            net = VCNet(cfg=cfg).to(device=device, non_blocking=True)
            swa_model = AveragedModel(net)
            state_dict = torch.load(early_stopping.save_dir / f"last-checkpoint_{fold}.pth")
            swa_model.load_state_dict(state_dict)
            logger.info("SWA Validation")
            swa_valid_assets = valid_per_epoch(
                cfg=cfg,
                model=swa_model,
                valid_loader=valid_loader,
                criterion=criterion,
            ...
            logger.info(f"{swa_valid_assets['valid_loss_avg']}")
            swa_mask = swa_valid_assets["mask_preds"]
            best_dice, best_th = calc_cv(mask_gt=valid_mask_gt, mask_pred=swa_mask)
            logger.info(f"SWA best dice: {best_dice} best th: {best_th}")
            wandb.log(
                {
                    f"fold{fold}_swa_best_valid_dice": best_dice,
                    f"fold{fold}_swa_best_valid_th": best_th,
                }
            )
            ...
            axes[3].imshow(swa_mask)
            axes[3].set_title("SWA Pred")
            fig.savefig(OUTPUT_DIR / exp_name / f"pred_mask_fold{fold}.png")

    # --- After running at all folds, report OOF score
    best_dices = [
        torch.load(CP_DIR / exp_name / f"best_fold{fold}.pth")["best_dice"]
        for fold in range(1, n_fold + 1)
...
def infer_per_sample(cfg: CFG, nets: list[VCNet], data: Data) -> np.ndarray:
    ...
    x = np.arange(0, p_w - crop_size + 1, stride)
    y = np.arange(0, p_h - crop_size + 1, stride)
    x, y = np.meshgrid(x, y)
