...
class CFG:
    # ================= Global cfg =====================
    exp_name = "exp015_fold5_Unet++_se_resnext50_32x4d_gradual_warmup_mixup_tverskyloss_lamb0.5"
    random_state = 42
    image_size = (224, 224)
    tile_size: int = 224
...
def train_per_epoch(
    ...
    with tqdm(enumerate(train_loader), total=len(train_loader)) as pbar:
        for i, (image, target) in pbar:
            ...
            wandb.log({f"fold{fold}_train_loss": loss.item(), "learning_rate": learning_rate})
...
def test(cfg: CFG, threshold: float = 0.4) -> pd.DataFrame:
    ...
    for fp in tqdm(test_data_path, total=len(test_data_path)):
        ...
        if IS_TRAIN:
            plt.imsave(
                OUTPUT_DIR / cfg.exp_name / f"test_pred_tile_image_{str(fp).replace('/', '_')}.png",
                pred_tile_image,
            )
            plt.close("all")
            plt.imsave(
                OUTPUT_DIR / cfg.exp_name / f"test_pred_tile_image_mask_{str(fp).replace('/', '_')}.png",
