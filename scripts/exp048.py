...
class CFG:
    # ================= Global cfg =====================
    exp_name = "exp048_fold5_Unet++_effb4_advprop_gradualwarm_mixup_tile224_slide74"
    random_state = 42
    tile_size: int = 224
    ...
    # if label_logits < cls_threshold, label = 0
    cls_threshold = 0.75
...
def valid_per_epoch(
    ...
    for step, (image, target) in tqdm(
        ...
        for i, (x1, y1, x2, y2) in enumerate(valid_xyxys[start_idx:end_idx]):
            ...
            cls_logits = pred_logtis[i].item()
            if cls_logits < cfg.cls_threshold:
                mask_preds[y1:y2, x1:x2] = np.zeros((cfg.tile_size, cfg.tile_size))
