...
- add a decode layer and pooling
...
class CFG:
    ...
    """BCESoftDiceLoss, DiceLoss, BCEDiceLoss, BCEWithLogitsLoss, TverskyLoss"""
    ...
    start_freaze_model_epoch: int = 5
    ...
    # --- Data Config
    fragment_z: tuple[int, int] = (28, 37)
    ...
    train_compose = [
        A.OneOf(
            [
                ...
                A.RandomResizedCrop(
                    crop_size, crop_size, scale=(0.5, 1.8), ratio=(0.5, 1.5)
                ),
            ]
        ),
        ...
        A.Cutout(
            p=0.5,
            num_holes=2,
            max_h_size=crop_size // 40,
            max_w_size=crop_size // 60,
            fill_value=0,
        ),
...
def read_image_mask(
    cfg: CFG, fragment_id: int, fragment_z: tuple[int, int]
...
def get_train_valid_split(
    ...
    for fragment_id in range(1, 4):
        # image: (h, w, in_chans)
        # mask: (h, w)
        image, mask = read_image_mask(
            cfg=cfg, fragment_id=fragment_id, fragment_z=fragment_z
        )
        logger.info(
        ...
        for y1 in y1_list:
            for x1 in x1_list:
                ...
                is_fold4 = (
                    fragment_id == 2
                    and valid_id == 4
                    and fragment_id_2_split_range[2] <= y2
                )
...
def get_loaders(
    ...
    train_loader = PrefetchLoader(
        train_loader, std=1, mean=0, channels=fragment_depth, fp16=use_amp
    )
    valid_loader = PrefetchLoader(
        valid_loader, std=1, mean=0, channels=fragment_depth, fp16=use_amp
    )
...
class SmpUnetDecorder(nn.Module):
    def __init__(
        self, in_channel: int, out_channels: list[int], skip_channels: list[int]
    ) -> None:
        super().__init__()
        ...
        logger.info(f"Decoder blocks: {self.blocks}")
...
class VCNet(nn.Module):
    def __init__(self, cfg: CFG) -> None:
        ...
        conv_dim = 64
        encoder1_dims = [conv_dim, 64, 128, 256, 512, 1024]
        decoder1_dims = [512, 256, 128, 64, 64]
        ...
        logger.info(f"Encoder1 weights: {self.weights1}")
        self.logit1 = nn.Conv2d(
            in_channels=decoder1_dims[-1], out_channels=1, kernel_size=1
        )

        encoder2_dims = [64, 128, 256, 512]
        decoder2_dims = [256, 128, 64, 32]
        self.encoder2 = resnet10t(
            pretrained=self.pretrained, in_chans=decoder1_dims[-1]
        )
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        x = e.layer4(x)
        encoder.append(x)
        # [mlp, pool, layer1, layer2, layer3, layer4]
...
# train funcions
# =================================================
def get_mask(cfg: CFG, fragment_id: int):
...
def training_fn(cfg: CFG) -> None:
    ...
    for fold in range(1, n_fold + 1):
        ...
        if resume_train:
            net.load_state_dict(
                torch.load(resume_weights[fold - 1], map_location=device)["state_dict"]
            )
        ...
        for epoch in range(epoch):
            ...
            if score > best_score:
                ...
                torch.save(
                    ...
                    str(CP_DIR / exp_name / f"best_fold{fold}.pth"),
                )
