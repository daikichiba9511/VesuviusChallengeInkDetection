...
- before mean, apply sigmoid to mask logits of model
...
class CFG:
    # ================= Global cfg =====================
    exp_name = "exp050_2_fold5_Unet++_effb4_advprop_gradualwarm_cutmix_mixup_tile224_slide28_rotate_tta_only"
    random_state = 42
    tile_size: int = 224
...
def valid_per_epoch(
    ...
    if cfg.use_tta:
        def _infer_with_sigmoid(x):
            x = model(x)
            x["pred_mask_logits"] = torch.sigmoid(x["pred_mask_logits"])
            return x
